-- =============================================
-- SAVI BUTCE v.1 - TAM VERITABANI KURULUMU
-- =============================================
...
INSERT INTO roles (rol_adi, aciklama) VALUES
    ('admin', 'Sistem Yoneticisi - Tum yetkilere sahip'),
    ('president', 'Okul Aile Birligi Baskani'),
    ('principal', 'Okul Muduru - Goruntuleme ve raporlama yetkileri')
ON CONFLICT (rol_adi) DO NOTHING;

-- IZINLER

INSERT INTO permissions (izin_adi, kategori, aciklama) VALUES
    ('view_dashboard', 'sayfa', 'Ana sayfa goruntuleme'),
    ('view_transactions', 'sayfa', 'Islemler sayfasi goruntuleme'),
    ('view_categories', 'sayfa', 'Kategoriler sayfasi goruntuleme'),
    ('view_students', 'sayfa', 'Ogrenciler sayfasi goruntuleme'),
    ('view_fees', 'sayfa', 'Aidatlar sayfasi goruntuleme'),
    ('view_reports', 'sayfa', 'Raporlar sayfasi goruntuleme'),
    ('view_admin', 'sayfa', 'Admin paneli goruntuleme'),
    ('create_transaction', 'islem', 'Islem ekleme'),
    ('edit_transaction', 'islem', 'Islem duzenleme'),
    ('delete_transaction', 'islem', 'Islem silme'),
    ('create_category', 'kategori', 'Kategori ekleme'),
    ('edit_category', 'kategori', 'Kategori duzenleme'),
    ('delete_category', 'kategori', 'Kategori silme'),
    ('create_student', 'ogrenci', 'Ogrenci ekleme'),
    ('edit_student', 'ogrenci', 'Ogrenci duzenleme'),
    ('delete_student', 'ogrenci', 'Ogrenci silme'),
    ('import_students', 'ogrenci', 'Toplu ogrenci ice aktarma'),
    ('create_fee', 'aidat', 'Aidat ekleme'),
    ('edit_fee', 'aidat', 'Aidat duzenleme'),
    ('delete_fee', 'aidat', 'Aidat silme'),
    ('manage_payments', 'aidat', 'Odeme yonetimi'),
    ('view_financial_reports', 'rapor', 'Mali raporlari goruntuleme'),
    ('view_student_reports', 'rapor', 'Ogrenci raporlari goruntuleme'),
    ('export_reports', 'rapor', 'Raporlari disa aktarma'),
    ('manage_users', 'admin', 'Kullanici yonetimi'),
    ('manage_roles', 'admin', 'Rol yonetimi'),
    ('manage_permissions', 'admin', 'Izin yonetimi')
ON CONFLICT (izin_adi) DO NOTHING;

-- Admin rolune tum izinleri ver
...
-- President rolune izinleri ver
INSERT INTO role_permissions (rol_id, izin_id)
SELECT r.id, p.id
FROM roles r CROSS JOIN permissions p
WHERE r.rol_adi = 'president'
AND p.izin_adi IN (
    'view_dashboard','view_transactions','view_categories','view_students',
    'view_fees','view_reports','create_transaction','edit_transaction',
    'delete_transaction','create_category','edit_category','delete_category',
    'create_student','edit_student','delete_student','import_students',
    'create_fee','edit_fee','delete_fee','manage_payments',
    'view_financial_reports','view_student_reports','export_reports'
)
ON CONFLICT (rol_id, izin_id) DO NOTHING;

-- Principal rolune goruntuleme izinleri ver
INSERT INTO role_permissions (rol_id, izin_id)
SELECT r.id, p.id
FROM roles r CROSS JOIN permissions p
...
AND p.izin_adi IN (
    'view_dashboard','view_transactions','view_categories','view_students',
    'view_fees','view_reports','view_financial_reports','view_student_reports',
    'export_reports'
)
ON CONFLICT (rol_id, izin_id) DO NOTHING;
...
SELECT
    'admin',
    '$2b$10$b0RP1yWK9C8yYTcBHLn13uRA29OvFDD5D2lGAcLbEy7HtA814qu4G',
    'Sistem Yoneticisi',
    'admin@example.com',
    r.id,
    true
